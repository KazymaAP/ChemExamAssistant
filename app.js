...
import { getParsed, setParsed } from './storageService.js';
        ...
        function smartSearch(query) {
            ...
            const filtered = allQuestions.filter(q => result.has(q.id));
            searchResultsCache.set(query, filtered);
            return filtered;
        }
        ...
        function importProgress(file) {
            if (!file) return;
            if (!confirm('Импорт перезапишет текущий прогресс. Продолжить?')) return;
        ...
        async function backupToIndexedDB() {
            if (!('indexedDB' in window)) {
                showToast('IndexedDB не поддерживается в этом браузере', 'info');
                return;
            }
            ...
            const request = indexedDB.open('ChemExamBackupDB', 1);
            ...
            request.onsuccess = () => {
                ...
                tx.objectStore('snapshots').add({ data: payload });
                tx.oncomplete = () => showToast('💾 Резервная копия сохранена в IndexedDB', 'success');
                tx.onerror = () => showToast('Ошибка сохранения резервной копии', 'danger');
            };
            request.onerror = () => showToast('Ошибка открытия IndexedDB', 'danger');
        }

        function checkDataIntegrity() {
            const issues = [];
            if (!Array.isArray(allQuestions) || allQuestions.length !== TOTAL_QUESTIONS) {
                issues.push('Некорректный банк вопросов');
            }
            if (!Array.isArray(bookmarks)) {
                issues.push('Некорректный формат закладок');
            }
            if (typeof notes !== 'object' || notes === null) {
                issues.push('Некорректный формат заметок');
            }
            const ids = new Set(allQuestions.map(q => q.id));
            if (ids.size !== allQuestions.length) {
                issues.push('Есть дубликаты id вопросов');
            }

            if (issues.length) {
                showToast(`⚠️ Найдены проблемы: ${issues.join('; ')}`, 'danger');
            } else {
                showToast('✅ Проверка целостности пройдена', 'success');
            }
        }
            ...
            app.innerHTML = `
                ...
                <main id="main-content" class="main-content scroll-container">
                    ...
                    <div class="card">
                        ...
                        <button class="btn btn-secondary" onclick="backupToIndexedDB()" style="margin-top:6px">💾 Резервная копия (IndexedDB)</button>
                        <button class="btn btn-secondary" onclick="checkDataIntegrity()" style="margin-top:6px">🩺 Проверить целостность данных</button>
                    </div>
